...
- [ ] CLB 正反器的 clock 來源可能可配置（全域時脈 or 本地 C 輸入）→ **已實作但精確度仍待確認（TASK-017，2026-07-08）**：`FPGA/FPGA.html` 新增 `clk_src`（'global'|'C'），選 'C' 時該 CLB 不再理會全域 `stepClock()`，改為每幀偵測自己 `in_C` 的 0→1 上升緣來鎖存（`updateLocalClocks()`）。**這個功能本身只有單一第三方 SystemC 重現專案佐證，未經原廠資料交叉確認**，維持待確認狀態；實作只是把「使用者選這個選項後會發生什麼」定義清楚，不代表已證實真實 XC2064 確有此特性。UI 與程式碼註解皆已標註此警告。
