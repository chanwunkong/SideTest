...
const bleManager = {
    MANUFACTURER_ID: 256,
    WEIGHT_OFFSET: 10,
    TIMEOUT_SECONDS: 4, // 建議改為 4 秒提升靈敏度
    ...
    async toggleConnection() {
        ...
        if (this.connectedDevice) { this.disconnect(); return; }
    ...
    handleAdvertisement(event) {
        ...
        if (data && data.byteLength >= this.WEIGHT_OFFSET + 2) {
            ...
            // 更新峰值力量
            if (this.currentWeight > this.peakWeight) this.peakWeight = this.currentWeight;
    ...
    resetAdvertisementTimeout() {
        ...
        this.advertisementTimeout = setTimeout(() => this.disconnect(), this.TIMEOUT_SECONDS * 1000);
    },
    ...
    updateButtonUI(state) {
        ...
        if (state === 'disconnected') {
            ...
            if (countdown) {
                countdown.classList.replace('text-[5rem]', 'text-[8rem]');
                countdown.classList.replace('md:text-[6rem]', 'md:text-[10rem]');
            }
            if (chartCanvas) { chartCanvas.style.height = '0'; chartCanvas.classList.add('opacity-0'); }
            ...
            if (countdown) {
                countdown.classList.replace('text-[8rem]', 'text-[5rem]');
                countdown.classList.replace('md:text-[10rem]', 'md:text-[6rem]');
            }
            if (chartCanvas) { chartCanvas.style.height = '150px'; chartCanvas.classList.remove('opacity-0'); }
