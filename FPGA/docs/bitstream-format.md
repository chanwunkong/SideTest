...
│ "XSIM"      │ = 1 或 2  │ = GRID_SIZE│ MSB-first，不足 8 bit 補 0 │
└─────────────┴───────────┴────────────┴───────────────────────────┘
```

- **magic**：固定 4 bytes `0x58 0x53 0x49 0x4D`（ASCII `"XSIM"`）。匯入時若不符即拒絕解析。
- **version**：`serializeBitstream()` 目前一律匯出 **版本 2**（TASK-015 新增全域長線欄位後的格式）；
  `deserializeBitstream()` 同時可讀版本 1（TASK-006/007 舊格式，無長線欄位）與版本 2，
  讀到版本 1 時，所有長線相關欄位一律套用預設值（`'wire'`/`false`），不會報錯，
  `FPGA/tests/` 下 TASK-007 產生的舊 `.bit` 檔仍可正常匯入。日後格式若再變動須再遞增版本號，
  並在 `deserializeBitstream()` 中依版本號分支處理，不得直接棄用舊版本讀取能力。
...
## 2. Payload 內容與順序（版本 2）

### 2.1 CLB 配置（row-major，r=0..size-1, c=0..size-1，每顆 25 bit）
...
版本 1 只有前 21 bit（無 `src_a`/`src_b`/`drive_h_long`/`drive_v_long`），讀取版本 1 時這 4 個欄位一律預設為 `'wire'`/`false`。
...
### 2.7 全域長線狀態（僅版本 2；版本 1 無此區塊，預設全部 `false`）
