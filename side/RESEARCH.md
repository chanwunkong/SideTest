...
### keyno 欄位缺失問題（2026-06-26 測試發現）
- **現象**：某次上傳 keyno 顯示「找不到」，但數值有進系統
- **釐清**：`0000030658` 是 Kevin 提供的測試 keyno（對應病患：謝小易），keyno 本身正確
- **截圖**：後台「今日檢測詳情」可見資料列（謝小易，體溫 36.2 / 36.9，心率 66，血氧 66），代表 keyno 有成功對應
- **可能原因**：某次送出時 keyno 欄位未正確帶入 JSON payload（例如病歷號欄位為空就送出）
- **待確認**：檢查 browser DevTools → Network，確認每次上傳的 `data` 欄位是否都有 keyno 值
